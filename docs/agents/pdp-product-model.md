...
| Donate                    | `variation`    | the three club presets; cart primaries only — any-amount giving is a standalone, non-cart affordance (presets split in the preset-selection grilling; the `$1` placeholder retired) |
...
- **Donations** — resolved in
  ...
  one record per preset, primaries the three club presets — detail in
