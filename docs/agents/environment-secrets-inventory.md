...
| `REVALIDATE_SECRET`                                 | the DatoCMS sender's credential in `src/app/api/revalidate/route.ts`, presented as `X-Revalidate-Secret` (ForgeCMS signs its body instead; DatoCMS cannot) | Cache webhook           | **Stay** — wired for the DatoCMS sender                                                |
