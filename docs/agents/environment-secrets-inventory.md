...
| `REVALIDATE_SECRET`                                 | reserved for the DatoCMS half of `src/app/api/revalidate/route.ts` (the ForgeCMS sender authenticates by HMAC instead) | Cache webhook           | **Stay** — still to be wired for the DatoCMS sender ([#103](https://github.com/Chapster87/pghrugby/issues/103)) |
| `CMS_WEBHOOK_SECRET`                                | `src/app/api/revalidate/route.ts` — verifies `X-ForgeCMS-Signature` (HMAC-SHA256 of the raw body)                      | ForgeCMS publish signal | **Stay** — site-side verification key; must equal the instance's Railway value                                  |
...
# Cache invalidation. The receiver is src/app/api/revalidate/route.ts.
# ForgeCMS authentications by HMAC signature; DatoCMS uses the shared secret below.
REVALIDATE_SECRET=
CMS_WEBHOOK_SECRET=
```
