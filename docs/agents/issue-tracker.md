...
Repository: `Chapster87/pghrugby`. Infer the repo from `git remote -v`; `gh` does
this automatically when run inside a clone.
...
When an agent needs to reference an issue, use the `#123` format.
